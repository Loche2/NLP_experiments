...
class Classifier(nn.Module):
    def __init__(self, hidden_size, embedding_size, vocab_size,
                 n_classes=14, n_layers=1, direction="bidirectional",
        ...
        self.embedding = nn.Embedding(num_embeddings=self.vocab_size, embedding_dim=self.embedding_size,
                                      _weight=nn.Parameter(torch.empty(self.vocab_size, self.embedding_size).uniform_(-self.init_scale, self.init_scale)))

        # 定义LSTM，它将用来编码网络
        self.lstm = nn.LSTM(input_size=self.embedding_size, hidden_size=self.
                            hidden_size, num_layers=self.n_layers, bidirectional=True, dropout=self.dropout_rate)
        ...
        self.cls_fc = nn.Linear(in_features=self.hidden_size * 2 if direction == "bidirectional" else hidden_size, out_features=self.n_classes)
...
if use_gpu:
    ...
    print("GPU is available. Moving model and data to GPU...")
else:
